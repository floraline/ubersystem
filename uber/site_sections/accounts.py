...
class Root:
    ...
    def can_spam(self, out, session):
        ...
        for a in session.query(Attendee).join(Attendee.cached_account).filter_by(can_spam=True).order_by('email').all():
    ...
    def staff_emails(self, out, session):
        out.writerow(["fullname", "email", "zipcode"])
        for a in session.query(Attendee).join(Attendee.cached_account).filter_by(staffing=True, placeholder=False).order_by('email').all():
            out.writerow([a.full_name, a.email, a.zip_code])
